...

{
    "inputs": [
        ...
        },
        {
            "id": "log_odds_score",
            "type": "float?",
            "doc": "The LOD threshold for us to call confidently a genotype"
        },
        {
            "id": "out",
            "type": "string?",
    ...
    "arguments": [
        {
            "valueFrom": "\t\tjava -jar $(inputs.gatk.path) \\\t\t\t-T GATKPaperGenotyper \\\t\t\t-R $(inputs.ref.path) \\\t\t\t--input_file $(inputs.input_file) \\\t\t\t$(\"--intervals \" + inputs.intervals) \\\t\t\t$(\"-nt\" + inputs.ntVal) \\\t\t\t-LOD $(inputs.log_odds_score) \\\t\t\t-o $(inputs.out) \\\t\t\t$(inputs.userString) \t",
            "shellQuote": false
        }
    ],
    "cwlVersion": "v1.0"
