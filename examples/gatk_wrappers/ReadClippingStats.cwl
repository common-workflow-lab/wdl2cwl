...

{
    "inputs": [
        ...
        },
        {
            "id": "include_unmapped",
            "type": "boolean?",
            "doc": "Include unmapped reads in the analysis"
        },
        {
            "id": "out",
            "type": "string?",
        ...
        },
        {
            "id": "skip",
            "type": "int?",
            "doc": "Do not print all reads, skip some."
        }
    ],
    "id": "ReadClippingStats",
    "baseCommand": [],
    ...
    "arguments": [
        {
            "valueFrom": "\t\tjava -jar $(inputs.gatk.path) \\\t\t\t-T ReadClippingStats \\\t\t\t-R $(inputs.ref.path) \\\t\t\t--input_file $(inputs.input_file) \\\t\t\t$(\"--intervals \" + inputs.intervals) \\\t\t\t-u $(inputs.include_unmapped) \\\t\t\t-o $(inputs.out) \\\t\t\t-skip $(inputs.skip) \\\t\t\t$(inputs.userString) \t",
            "shellQuote": false
        }
    ],
    "cwlVersion": "v1.0"
