...

{
    ...
    "arguments": [
        {
            "valueFrom": "\t\tjava -jar $(inputs.gatk.path) \\\t\t\t-T CountRODs \\\t\t\t-R $(inputs.ref.path) \\\t\t\t$(\"--intervals \" + inputs.intervals) \\\t\t\t$(\"-nct\" + inputs.nctVal) \\\t\t\t$(\"-nt\" + inputs.ntVal) \\\t\t\t-o $(inputs.out) \\\t\t\t-rod $(inputs.rod) \\\t\t\t-s $(inputs.showSkipped) \\\t\t\t-v $(inputs.verbose) \\\t\t\t$(inputs.userString) \t",
            "shellQuote": false
        }
    ],
    "cwlVersion": "v1.0"
