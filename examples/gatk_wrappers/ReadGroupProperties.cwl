...

{
    "inputs": [
        ...
        {
            "id": "max_values_for_median",
            "type": "int?",
            "doc": "Calculate median from the first maxElementsForMedian values observed"
        },
        {
            "id": "out",
            "type": "string?",
    ...
    "arguments": [
        {
            "valueFrom": "\t\tjava -jar $(inputs.gatk.path) \\\t\t\t-T ReadGroupProperties \\\t\t\t-R $(inputs.ref.path) \\\t\t\t--input_file $(inputs.input_file) \\\t\t\t-maxElementsForMedian $(inputs.max_values_for_median) \\\t\t\t-o $(inputs.out) \\\t\t\t$(inputs.userString) \t",
            "shellQuote": false
        }
    ],
    "cwlVersion": "v1.0"
