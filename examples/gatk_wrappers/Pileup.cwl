...

{
    "inputs": [
        ...
        },
        {
            "id": "metadata",
            "type": "string[]?",
            "doc": "ROD file containing metadata"
        },
        {
            "id": "out",
            "type": "string?",
        ...
        },
        {
            "id": "showVerbose",
            "type": "boolean?",
            "doc": "Add an extra verbose section to the pileup output"
        }
    ],
    "id": "Pileup",
    "baseCommand": [],
    ...
    "arguments": [
        {
            "valueFrom": "\t\tjava -jar $(inputs.gatk.path) \\\t\t\t-T Pileup \\\t\t\t-R $(inputs.ref.path) \\\t\t\t--input_file $(inputs.input_file) \\\t\t\t$(\"--intervals \" + inputs.intervals) \\\t\t\t$(\"-nct\" + inputs.nctVal) \\\t\t\t$(\"-nt\" + inputs.ntVal) \\\t\t\t-metadata $(inputs.metadata) \\\t\t\t-o $(inputs.out) \\\t\t\t-verbose $(inputs.showVerbose) \\\t\t\t$(inputs.userString) \t",
            "shellQuote": false
        }
    ],
    "cwlVersion": "v1.0"
